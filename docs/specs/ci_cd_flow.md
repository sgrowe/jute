...
On new pushes to any branch a github action should run the same test command over the same matrix as the release flow above. It should also build for each platform and output the binary file sizes it the same way. It should NOT create a new github release or otherwise publish anything however.
